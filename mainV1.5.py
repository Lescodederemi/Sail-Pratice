...
async def on_ready():  
    ...
    Users.create_table()
    Users.create_suivi_table()  # Ajoutez cette ligne
        ...
        async def cours_select_callback(interaction: discord.Interaction):
            ...
            async def niveau_select_callback(interaction: discord.Interaction):
                ...
                async def exercice_select_callback(interaction: discord.Interaction):
                    ...
                    # Récupérer le média associé à l'exercice
                    media_info = exercices.get_media_for_exercice(ex_id)
...
async def show_chapitre(interaction: discord.Interaction):
    ...
    if is_last_chapter:
        if learn.is_course_completed(interaction.user.id, cours_id):
            ...
            # Afficher le bouton "Valider le cours" au lieu de lancer automatiquement
            qcm_button = Button(
...
async def show_qcm(interaction: discord.Interaction):
    ...
    embed.set_footer(text="Score minimum requis: 50%")
    
    # Récupérer le média associé à la question QCM
    media_info = qcm.get_media_for_qcm(qcm_item['id'])
    if media_info:
        media_type = media_info['type']
        media_url = media_info['url']
...
async def evaluate_qcm(interaction: discord.Interaction):
    ...
    # Fixer le score minimum à 50% pour tous les cours
    score_min = 50
...
# Gestion des médias
@bot.tree.command(name="pj_ajouter", description="Ajoute un média et le lie directement à un élément")
@app_commands.default_permissions(administrator=True)
@app_commands.describe(
    type_pj="Type de pièce jointe",
    url="URL du média",
    element_type="Type d'élément à lier",
...
@app_commands.choices(element_type=[
    app_commands.Choice(name="Cours", value="cours"),
    app_commands.Choice(name="Exercice", value="exercice"),
    app_commands.Choice(name="QCM", value="qcm"),
    app_commands.Choice(name="Chapitre", value="chapitre")
])
async def ajouter_pj(interaction: discord.Interaction, 
                    type_pj: app_commands.Choice[str], 
                    url: str,
                    element_type: app_commands.Choice[str],
                    element_id: int):
    ...
    if not media_id:
        await interaction.response.send_message(
            "❌ Erreur lors de l'ajout du média à la bibliothèque.",
            ephemeral=True
        )
        return
    
    embed = discord.Embed(
        title="✅ Média ajouté et lié",
        description=f"ID: `{media_id}`\nType: `{type_pj.name}`",
        color=discord.Color.green()
    )
    embed.add_field(name="URL", value=url, inline=False)
    
    # Lier IMMÉDIATEMENT le média à l'élément spécifié
    success = False
    element_name = ""
    
    try:
        if element_type.value == "cours":
            success = learn.link_media_to_cours(element_id, media_id)
            element_name = f"cours #{element_id}"
        elif element_type.value == "exercice":
            success = exercices.link_media_to_exercice(element_id, media_id)
            element_name = f"exercice #{element_id}"
        elif element_type.value == "qcm":
            success = qcm.link_media_to_qcm(element_id, media_id)
            element_name = f"QCM #{element_id}"
        elif element_type.value == "chapitre":
            success = learn.link_media_to_chapitre(element_id, media_id)
            element_name = f"chapitre #{element_id}"
        
        if success:
            embed.add_field(
                name="🔗 Liaison réussie",
                value=f"Média lié au {element_name}",
                inline=False
            )
        else:
            embed.add_field(
                name="⚠️ Avertissement",
                value=f"Impossible de lier le média au {element_name}. Vérifiez l'ID.",
                inline=False
            )
            # Si la liaison échoue, on supprime le média créé
            learn.delete_media(media_id)
            embed.add_field(
                name="🗑️ Média supprimé",
                value="Le média a été supprimé suite à l'échec de la liaison.",
                inline=False
            )
        ...
        embed = discord.Embed(
            ...
            description=f"Erreur lors de la liaison: {str(e)}",
            color=discord.Color.red()
        )
        # En cas d'erreur, on supprime le média créé
        learn.delete_media(media_id)
